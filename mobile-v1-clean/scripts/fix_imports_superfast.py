...
def process_file(file_path):
    ...
    def replacer(match):
        ...
        new_import_str = resolve_import(Path(file_path), import_str)
    ...
    if new_content != content:
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        print(f"Updated {Path(file_path).relative_to(BASE_DIR)}")

def main():
    for root, dirs, files in os.walk(BASE_DIR):
        # Prevent traversal into heavy/unnecessary directories
