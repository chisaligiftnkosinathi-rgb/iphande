...
export default function BusinessProfileScreen() {
    const slug = 'mandla-auto'; // V1 preview slug until routing is wired
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                const response = await fetch(`${API_BASE_URL}/public/business/${slug}`);
                if (response.ok) {
                ...
                setProfile({
                    slug: slug || 'monica-twala',
                    name: 'Monica Twala Cleaning Services',
                    location_string: 'Daveyton, Gauteng',
                    steward_story: 'I started this business to create clean, dignified spaces for families, and to ensure my children could finish their schooling. Every home we clean is treated with respect and prayer.',
                    whatsapp_number: '27710000000',
                    cover_photo_url: 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?q=80&w=1000&auto=format&fit=crop',
                    logo_url: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=200&auto=format&fit=crop',
                    supporting_images: [
                    ...
                    services: ['Deep Cleaning', 'Move-in/Move-out', 'Office Sanitation'],
                    opportunities: [
                        { id: '1', title: 'Assistant Cleaner', archetype: 'work' },
                        { id: '2', title: 'Referral Partner', archetype: 'sales' }
                    ]
                });
            } finally {
                setLoading(false);
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
            ...
            <View style={styles.orbitGrid}>
                ...
                <View style={styles.orbitCard}>
                    <View style={[styles.geometricBadge, { backgroundColor: '#E8F5E9' }]} />
                ...
                <View style={styles.orbitCard}>
                    <View style={[styles.geometricBadge, { backgroundColor: '#E3F2FD' }]} />
...
const styles = StyleSheet.create({
    ...
    avatarRing: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
    avatar: { width: 110, height: 110, borderRadius: 55 },
    businessName: { fontSize: 26, fontWeight: '800', color: '#2C3E50', marginTop: 12, textAlign: 'center', letterSpacing: 0.5 },
    locationText: { fontSize: 15, color: '#7F8C8D', marginTop: 4, fontWeight: '500' },

    // Orbit Base
    sectionTitle: { fontSize: 14, fontWeight: '700', color: '#95A5A6', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 12, textAlign: 'center' },
    ...
    orbitCard: { flex: 1, backgroundColor: '#fff', borderRadius: 16, padding: 16, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 2, position: 'relative' },
    geometricBadge: { width: 40, height: 40, borderRadius: 20, position: 'absolute', top: -20, zIndex: 5 },
    orbitTitle: { fontSize: 18, fontWeight: '700', color: '#2C3E50', marginTop: 12, marginBottom: 12 },
    orbitItem: { fontSize: 15, color: '#555', marginBottom: 6, textAlign: 'center' },

    opportunityPill: { backgroundColor: '#F8F9F9', width: '100%', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, marginBottom: 8, alignItems: 'center', borderWidth: 1, borderColor: '#EAEDED' },
    opportunityText: { fontSize: 14, fontWeight: '600', color: '#2C3E50', textAlign: 'center' },
    archetypeBadge: { fontSize: 10, color: '#2980B9', textTransform: 'uppercase', fontWeight: '800', marginTop: 4 },
