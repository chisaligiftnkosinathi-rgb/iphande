...
export async function fetchPublicProfile(slug: string): Promise<Profile> {
    return apiGet<Profile>(`public/${slug}`);
}
...
export async function updateProfileLocation(profileId: string, payload: {
    location?: string;
    short_bio?: string;
}): Promise<Profile> {
    return apiPatch<typeof payload, Profile>(`profiles/${profileId}/location`, payload);
}
